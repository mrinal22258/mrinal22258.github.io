...
const FrameScrollAnimation = ({ frameCount = 240 }) => {
  ...
  const currentFrameUrl = (index) => `/images/ezgif-frame-${String(index + 1).padStart(3, "0")}.jpg`;

  // Preloading Logic
  useEffect(() => {
    preloadImageSequence({
      frameCount,
      currentFrameUrlFn: currentFrameUrl,
      criticalCount: 15,
      batchSize: 5,
      onProgress: (progress) => setLoadingProgress(progress),
      onCriticalLoaded: () => setLoaded(true),
      imagesArrayRef: images
    });
  }, [frameCount]);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const img = getNearestLoadedFrame(index, images.current);
