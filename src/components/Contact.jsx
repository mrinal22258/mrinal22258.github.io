...
import { preloadImageSequence, getNearestLoadedFrame } from "../utils/preloadHelper";
...
const Contact = () => {
  ...
  // 1. Preload Sequence progressively
  useEffect(() => {
    preloadImageSequence({
      frameCount,
      currentFrameUrlFn: currentFrame,
      criticalCount: 15,
      batchSize: 5,
      onProgress: (progress) => setLoadingProgress(progress),
      onCriticalLoaded: () => setLoaded(true),
      imagesArrayRef: imagesRef
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const img = getNearestLoadedFrame(frameIdx, imagesRef.current);
      if (img) {
          ...
          >
            <div className="text-center mb-8">
              ...
              <div className="flex items-center justify-center space-x-2 text-cyan-500/60 font-mono text-[9px] tracking-[0.6em] uppercase">
                <FiShield />
                <span>mrinal22258@iiitd.ac.in · +91-6205435300</span>
              </div>
            </div>
